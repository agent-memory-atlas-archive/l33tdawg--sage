...
`Commit` (`app.go:10103-10266`) runs after `FinalizeBlock` for each block. It:
...
`processMemoryVote` (`app.go:6421-6565`):
...
`checkAndApplyQuorum` (`app.go:6566-6797`):
...
1. **Quorum failure**: all validators voted, `acceptWeight / totalWeight < 2/3` → deprecated in `checkAndApplyQuorum` (`app.go:6566`).
