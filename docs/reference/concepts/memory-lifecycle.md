...
`FinalizeBlock` (`app.go:3913-3986`) is the deterministic execution path, delegating to `finalizeBlockUncommitted` (`app.go:3987`). Key constraint from the code comment: **"CRITICAL: No time.Now(), no map iteration without sorting, no goroutines, and no external I/O except the supplied BadgerStore view."** Block time from `req.Time` is used for all timestamps.

`processMemorySubmit` (`app.go:5114-5657`) does:
...
`processMemoryVote` (`app.go:6452-6596`):
...
`checkAndApplyQuorum` (`app.go:6597-6828`):
...
**app-v17 - quorum-scaled two-phase challenge + reinstate.** Ships dormant behind the app-v17 fork and activates only via the governed upgrade ladder (`postAppV17Rules`, strict `>`); every pre-fork block and the activation block itself replay byte-identically. Once active, `processMemoryChallenge` (`app.go:6976-7610`) branches on the memory's domain:
...
1. **Quorum failure**: all validators voted, `acceptWeight / totalWeight < 2/3` → deprecated in `checkAndApplyQuorum` (`app.go:6597`).
