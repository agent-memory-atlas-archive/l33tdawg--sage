...
`FinalizeBlock` (`app.go:3902-3969`) is the deterministic execution path, delegating to `finalizeBlockUncommitted` (`app.go:3980`). Key constraint from the code comment: **"CRITICAL: No time.Now(), no map iteration without sorting, no goroutines, and no external I/O except the supplied BadgerStore view."** Block time from `req.Time` is used for all timestamps.

`processMemorySubmit` (`app.go:5107-5650`) does:
...
`Commit` (`app.go:10127-10290`) runs after `FinalizeBlock` for each block. It:
...
`processMemoryVote` (`app.go:6445-6589`):
...
`checkAndApplyQuorum` (`app.go:6590-6821`):
...
1. **Quorum failure**: all validators voted, `acceptWeight / totalWeight < 2/3` → deprecated in `checkAndApplyQuorum` (`app.go:6590`).
